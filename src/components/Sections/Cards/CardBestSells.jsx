...
function CardBestSells({offer_text, offer_text_coller,offer_bg, category, itemName, rating, vender, offerPrice, mrp, sold, totalAvailabe, imgName }) {
  return (
    <>
      <div className='relative border-2 border-solid rounded-md border-gray-200 h-auto w-60 mx-4'>
        <p className={`absolute -top-[2px] -left-[2px]  bg-[${offer_bg}] text-${offer_text_coller} px-3 py-2 rounded-tl-md rounded-br-md text-sm font-quicksand-w600`}>{offer_text}</p>
        <img className='w-full h-[15rem] bg-white' src={`/src/assets/Images/${imgName}.png`} alt="Coffe" />
